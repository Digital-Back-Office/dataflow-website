---
header:
  logo:
    href: "/"
    image: "/assets/images/dataflow-logo-green.svg"
    alt: "Dataflow logo"
  nav:
    desktop:
      - label: "Product"
        type: "mega"
        width: "w-[680px]"
        columns:
          - heading: "Features"
            span: 3
            items:
              - title: "Managed Dependencies"
                icon-id: "python"
                description: "Say goodbye to dependency hell"
                href: "/product/managed-dependencies"
              - title: "Development-Ready Workspace"
                icon-id: "workspace"
                description: "Start building instantly"
                href: "/product/development-ready-workspace"
              - title: "Shared Foundation"
                icon-id: "foundation"
                description: "One foundation, shared everywhere"
                href: "/product/shared-foundation"
              - title: "Deploy to Production"
                icon-id: "deploy"
                description: "Ship with confidence"
                href: "/product/deploy-to-production"
              - title: "Cloud-Agnostic"
                icon-id: "cloud"
                description: "Run on any cloud platform"
                href: "/product/cloud-agnostic"
          - heading: "Product"
            span: 2
            items:
              - title: "Interactive Demo"
                icon-id: "demo"
                description: "Interactive product walkthrough"
                action: "open-demo-modal"
      - label: "Solutions"
        type: "mega"
        width: "w-[760px]"
        columns:
          - heading: "Solutions"
            span: 2
            items:
              - title: "Enterprise"
                icon-id: "enterprise"
                description: "For large teams"
                href: "/solutions/enterprise"
              - title: "Startups & SMEs"
                icon-id: "startups"
                description: "Scale fast, start lean"
                href: "/solutions/startups-smes"
              - title: "Personal"
                icon-id: "personal"
                description: "Free for learning & projects"
                href: "/solutions/personal"
          - heading: "Comparisons"
            span: 3
            items:
              - title: "Dataflow vs Amazon SageMaker"
                icon-id: "sagemaker"
                description: "Compare SageMaker with Dataflow"
                href: "/compare/dataflow-vs-sagemaker"
              - title: "Dataflow vs Deepnote"
                icon-id: "deepnote"
                description: "Compare Deepnote with Dataflow"
                href: "/compare/dataflow-vs-deepnote"
              - title: "Dataflow vs Hex"
                icon-id: "hex"
                description: "Compare Hex with Dataflow"
                href: "/compare/dataflow-vs-hex"
              - title: "Dataflow vs Dataiku"
                icon-id: "dataiku"
                description: "Compare Dataiku with Dataflow"
                href: "/compare/dataflow-vs-dataiku"
              - title: "See all comparisons"
                icon-id: "compare"
                description: "Compare Dataflow with other apps"
                href: "/compare"
      - label: "Community"
        type: "mega"
        width: "w-[600px]"
        columns:
          - heading: "Resources"
            span: 3
            items:
              - title: "Docs"
                icon-id: "docs"
                description: "Explore dataflow documentation"
                href: "https://docs.dataflow.zone/"
              - title: "Blog"
                icon-id: "blog"
                description: "Data engineering insights"
                href: "/blog"
              - title: "Pressroom"
                icon-id: "press"
                description: "Official announcements & media"
                href: "/press"
              - title: "YouTube"
                icon-id: "youtube"
                description: "Video tutorials"
                href: "https://www.youtube.com/@DataFlowZone"
          - heading: "Connect"
            span: 2
            items:
              - title: "X"
                icon-id: "x"
                description: "Follow us"
                href: "https://x.com/dataflow_zone"
              - title: "LinkedIn"
                icon-id: "linkedin"
                description: "Follow updates"
                href: "https://www.linkedin.com/company/dataflow-zone/about/"
              - title: "Discord"
                icon-id: "discord"
                description: "Join community"
                href: "https://discord.gg/DCDPMphfth"
      - label: "Pricing"
        type: "link"
        href: "/pricing"
  ctas:
    primary:
      label: "Book Demo"
      href: "/book-demo"
    secondary:
      label: "Get Started"
      href: "https://app.dataflow.zone"
      target: "_blank"

footer:
  finance: "Our dedicated team is prepared to assist you."
  finance_underline: "Talk to our finance agent"
  description: "The unified operating system for data engineering. Orchestrate with Airflow, develop in VS Code, and deploy to any cloud—all without Python dependency conflicts."
  copyrights: "© All rights reserved."
  sections:
    - heading: "Product"
      links:
        - label: "Managed Dependencies"
          href: "/product/managed-dependencies"
        - label: "Development-Ready Workspace"
          href: "/product/development-ready-workspace"
        - label: "Shared Foundation"
          href: "/product/shared-foundation"
        - label: "Deploy to Production"
          href: "/product/deploy-to-production"
        - label: "Cloud-Agnostic"
          href: "/product/cloud-agnostic"

    - heading: "Solutions"
      links:
        - label: "Enterprise"
          href: "/solutions/enterprise"
        - label: "Startups & SMEs"
          href: "/solutions/startups-smes"
        - label: "Personal"
          href: "/solutions/personal"

    - heading: "Comparisons"
      links:
        - label: "Dataflow vs Amazon SageMaker"
          href: "/compare/dataflow-vs-sagemaker"
        - label: "Dataflow vs Deepnote"
          href: "/compare/dataflow-vs-deepnote"
        - label: "Dataflow vs Hex"
          href: "/compare/dataflow-vs-hex"
        - label: "Dataflow vs Dataiku"
          href: "/compare/dataflow-vs-dataiku"
        - label: "See all comparisons"
          href: "/compare"

    - heading: "Resources"
      links:
        - label: "Blog"
          href: "/blog"
        - label: "YouTube"
          href: "https://www.youtube.com/@DataFlowZone"
        - label: "Community"
          href: "https://discord.gg/DCDPMphfth"
        - label: "Documentation"
          href: "https://docs.dataflow.zone/"

    - heading: "Contact info"
      links:
        # - label: "+1 526 220 0444"
        #   href: "tel:+15262200444"
        - label: "support@dataflow.zone"
          href: "mailto:support@dataflow.zone"

    - heading: "Quick links"
      links:
        - label: "Book Demo"
          href: "/book-demo"
        - label: "Contact Us"
          href: "/contact-us"

    - heading: "Company"
      links:
        - label: "About us"
          href: "/about-us"
        - label: "Our team"
          href: "/about-us#team"
        - label: "Pressroom"
          href: "/press"


  bottom_bar:
    links:
      - label: "Privacy Policy"
        href: "/legal/privacy-policy"
      - label: "Terms and Conditions"
        href: "/legal/terms-and-conditions"

        
  social_links:
    - platform: "linkedin"
      url: "https://www.linkedin.com/company/dataflow-zone/about/"
      icon: "/assets/icons/icon-linkedin.svg"
    - platform: "X"
      url: "https://x.com/dataflow_zone"
      icon: "/assets/icons/icon-x.svg"
    - platform: "youtube"
      url: "https://www.youtube.com/@DataFlowZone"
      icon: "/assets/icons/icon-youtube.svg"
    # - platform: "Discourse"
    #   url: "https://discourse.dataflow.com"
    #   icon: "/assets/icons/icon-discourse.svg"
---
